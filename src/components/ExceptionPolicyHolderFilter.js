...
class ExceptionPolicyHolderFilter extends Component {
  _filterValue = (k) => {
      ...
      <Grid container className={classes.form}>
        <Grid item xs={3} className={classes.item}>
          <TextInput
            ...
            value={this._filterValue("camuCode")}
            onChange={(v) =>
              this._onChangeStringFilter("insuree_camuCode", v, STARTS_WITH_LOOKUP)
            }
          />
        </Grid>
...
export default withModulesManager(
  injectIntl(
    withTheme(
      withStyles(styles)(connect(mapStateToProps, null)(ExceptionPolicyHolderFilter))
    )
  )
);
