...
class PolicyHolderApprovalForm extends Component {
  ...
  _approveorreject = async (paymentData) => {
    try {
      ...
      if (!response?.payload?.data?.policyholderApproval?.success) {
        throw new Error(
          response?.payload?.data?.policyholderApproval?.message ||
      ...
      const severity = paymentData.status == 5 ? "success" : "error";
      const message =
  ...
  _rework = async (paymentData) => {
    const response = await this.props.PolicyholderReworkAction(
      "mm",
      paymentData
    );
  ...
  back = () => {
    historyPush(
      this.props.modulesManager,
      this.props.history,
      "policyHolder.route.policyholderRequest"
    );
  };
  ...
  render() {
    ...
    const exceptionApprove =
      !!approverData && policyHolder?.status === "Pending";
    return (
      <Fragment>
        {/* <Helmet
        ...
        <Form
          module="policyHolder"
          title={`N° de la demande. ${
            policyHolder?.requestNumber ? policyHolder?.requestNumber : ""
          }`}
