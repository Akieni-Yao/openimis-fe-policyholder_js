...
class PolicyHolderContributionPlanBundleSearcher extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    // setTimeout(() => {
    //   this.props.fetchPickerPolicyHolderContributionPlanBundles(
    //     this.props.modulesManager,
    //     [`policyHolder_Id: "${decodeId(this.props.policyHolder.id)}"`]
    //   );
    // }, 2000);
    if (
  ...
  fetch = (params) =>
    this.props.fetchPolicyHolderContributionPlanBundles(
      this.props.modulesManager,
      params
    );
    ...
    let result = [
      (policyHolderContributionPlanBundle) =>
        !!policyHolderContributionPlanBundle.contributionPlanBundle
          ? `${policyHolderContributionPlanBundle.contributionPlanBundle?.code} - ${policyHolderContributionPlanBundle.contributionPlanBundle?.name}`
          : // ? <PolicyHolderContributionPlanBundlePicker
            ...
            "",
      (policyHolderContributionPlanBundle) =>
        !!policyHolderContributionPlanBundle.jsonExt
          ? policyHolderContributionPlanBundle.jsonExt
          : "",
      (policyHolderContributionPlanBundle) =>
        ...
        (policyHolderContributionPlanBundle) =>
          !this.isDeletedFilterEnabled(policyHolderContributionPlanBundle) && (
            <UpdatePolicyHolderContributionPlanBundleDialog
              ...
              onSave={onSave}
              disabled={
                this.state.deleted.includes(
                  policyHolderContributionPlanBundle.id
                ) || this.isReplaced(policyHolderContributionPlanBundle)
              }
              isReplacing={true}
            />
          )
        ...
        (policyHolderContributionPlanBundle) =>
          ...
          withTooltip(
            <div>
              <IconButton
                onClick={() =>
                  this.onDelete(policyHolderContributionPlanBundle)
                }
                disabled={this.state.deleted.includes(
                  policyHolderContributionPlanBundle.id
                )}
              >
                <DeleteIcon />
    ...
    let confirm = () =>
      coreConfirm(
        formatMessageWithValues(
          ...
          {
            code: policyHolderContributionPlanBundle.contributionPlanBundle
              .code,
            name: policyHolderContributionPlanBundle.contributionPlanBundle
              .name,
          }
        ),
  ...
  isReplaced = (policyHolderContributionPlanBundle) =>
    !!policyHolderContributionPlanBundle.replacementUuid;

  isDeletedFilterEnabled = (policyHolderContributionPlanBundle) =>
    policyHolderContributionPlanBundle.isDeleted;

  isRowDisabled = (_, policyHolderContributionPlanBundle) =>
    this.state.deleted.includes(policyHolderContributionPlanBundle.id) &&
    !this.isDeletedFilterEnabled(policyHolderContributionPlanBundle);
  ...
  render() {
    const {
      intl,
      fetchingPolicyHolderContributionPlanBundles,
      fetchedPolicyHolderContributionPlanBundles,
      errorPolicyHolderContributionPlanBundles,
      policyHolderContributionPlanBundles,
      policyHolderContributionPlanBundlesPageInfo,
      policyHolderContributionPlanBundlesTotalCount,
    } = this.props;
    return (
      <Fragment>
        <Searcher
          ...
          tableTitle={formatMessageWithValues(
            intl,
            "policyHolder",
            "policyHolderContributionPlanBundle.searcher.title",
            { policyHolderContributionPlanBundlesTotalCount }
          )}
...
const mapStateToProps = (state) => ({
  fetchingPolicyHolderContributionPlanBundles:
    state.policyHolder.fetchingPolicyHolderContributionPlanBundles,
  fetchedPolicyHolderContributionPlanBundles:
    state.policyHolder.fetchedPolicyHolderContributionPlanBundles,
  errorPolicyHolderContributionPlanBundles:
    state.policyHolder.errorPolicyHolderContributionPlanBundles,
  policyHolderContributionPlanBundles:
    state.policyHolder.policyHolderContributionPlanBundles,
  policyHolderContributionPlanBundlesPageInfo:
    state.policyHolder.policyHolderContributionPlanBundlesPageInfo,
  policyHolderContributionPlanBundlesTotalCount:
    state.policyHolder.policyHolderContributionPlanBundlesTotalCount,
