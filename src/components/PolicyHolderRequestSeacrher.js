  ...
  itemFormatters = () => {
    ...
    const getStatusStyle = (status) => {
      ...
      switch (status) {
        ...
        case "Pending":
          return { color: "#FF841C", fontWeight: "bold" };
        default:
          ...
          return (
            <Grid item>
              ...
              {formatMessage(
                this.props.intl,
                "policyHolder",
                `policyHolder.requeststatus.${policyHolderUser.status}`
              )}
            </span>
              {policyHolderUser?.rejectedReason && (
                <Tooltip
                  arrow
                  title={this.rejectedCommentsTooltip(
                    policyHolderUser?.rejectedReason
                  )}
                >
                  <IconButton>
...
export default withModulesManager(
  injectIntl(
    connect(mapStateToProps, mapDispatchToProps)(PolicyHolderRequestSeacrher)
  )
);
